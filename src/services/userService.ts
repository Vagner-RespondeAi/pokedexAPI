...
export async function signUp(newUser: NewUser) {
  ...
  const alredyRegistered = await repository.findOne({
...
export async function signIn(user: User): Promise<string> {
  ...
  if (
    !alredyRegistered ||
    !bcrypt.compareSync(user.password, alredyRegistered.password)
  )
    return null;
  const session = await getRepository(Session).insert({
    userId: alredyRegistered.id,
  });
